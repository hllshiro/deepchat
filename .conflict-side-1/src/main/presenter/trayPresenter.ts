...
export class TrayPresenter {
  ...
  private createTray() {
    ...
    if (process.platform === 'darwin') {
      ...
      console.log(`[ICON] ${path.join(this.iconPath, 'win_tray.ico')}`)
    } else {
      // Linux 和其他平台
    ...
    // log
    console.log(`[ICON] empty: ${image.isEmpty()} size: ${JSON.stringify(image.getSize())}`)

    this.tray = new Tray(image)
    this.tray.setToolTip('Star-Agent')
