...
export class WindowPresenter implements IWindowPresenter {
  ...
  async createShellWindow(options?: {
    ...
    // 根据平台选择图标
    const iconPath = process.platform === 'win32' ? iconWin : icon
    const iconFile = nativeImage.createFromPath(iconPath)

    // log
    console.log(
      `[ICON] iconPath: ${iconPath} empty: ${iconFile.isEmpty()} size: ${JSON.stringify(iconFile.getSize())}`
    )
